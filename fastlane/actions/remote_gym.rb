module Fastlane
  module Actions
    ...
    class RemoteGymAction < Action

      APP_PATH = 'iosapp_with_snapshot' # TODO extract from options[:project]   
      CI_PROVIDER = 'travis' # TODO Extract this ci_provider stuff into the server
      ACTION = 'gym'
      ...
      def self.output_log(ci_provider, log, action)    
        ...
        relevant_log = ''
        keep = false
        start_line = /Cruising over to lane 'build'/ # TODO action param
        end_line = /Cruising back to lane 'remote_gym'/ # TODO action param
